...
all_tests() ->
    [single_node_write,
     ...
     committed_offset_calculate_with_single_node,
     committed_offset_calculate_with_cluster,
     cluster_should_work_with_committed_offset_calculate_on_off].
...
committed_offset_calculate_with_single_node(Config) ->
    Name = ?config(cluster_name, Config),
...
committed_offset_calculate_with_cluster(Config) ->
    PrivDir = ?config(data_dir, Config),
    Name = ?config(cluster_name, Config),
    PeerStates = [start_child_node(N, PrivDir, application:get_all_env(osiris))
                  || N <- [s1, s2, s3]],
...
cluster_should_work_with_committed_offset_calculate_on_off(Config) ->
    PrivDir = ?config(data_dir, Config),
    Name = ?config(cluster_name, Config),
    PeerStates = [start_child_node(N, PrivDir, application:get_all_env(osiris))
                  || N <- [s1, s2, s3]],
